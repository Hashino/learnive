...
mod tests {
    ...
    async fn reading_interactions_ask_annotate_and_read_to_end() {
        ...
        let (status, body) = call(authed(
            ...
            &format!(
                r#"{{"question":"why <does> this work?","anchor":{{"block_id":"{block_id}"}}}}"#
            ),
        ))
        .await;
        assert_eq!(status, StatusCode::OK);
    ...
    async fn objective_and_plan_endpoints_work_in_demo_mode() {
        ...
        let (status, body) = call(authed(
            "POST",
            "/api/documents",
            r#"{"topic":"fractions","objective_text":"Learn to add and subtract fractions","non_goals":["mixed numbers"]}"#,
        ))
        .await;
        assert_eq!(status, StatusCode::OK);
        let created: serde_json::Value = serde_json::from_str(&body).unwrap();
        ...
        let (status, body) = call(authed(
            ...
            r#"{"text":"Learn to add, subtract, and simplify fractions","non_goals":[]}"#,
        ))
        .await;
        assert_eq!(status, StatusCode::OK);
