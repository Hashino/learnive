...
// learnive patch (see vendor/pdf-extract/PATCH.md): minimum operand count
// `Processor::process_stream`'s match block actually reads for each
// content-stream operator it handles, used to skip a malformed/truncated
...
fn min_operands(op: &str) -> usize {
    match op {
        ...
        "m" | "l" | "Td" | "TD" => 2,
        "v" | "y" | "re" => 4,
        "CS" | "cs" | "gs" | "Do" | "TJ" | "Tj" | "Tc" | "Tw" | "Tz" | "TL" | "Ts" | "w" => 1,
        _ => 0,
    }
}
...
impl<'a> Processor<'a> {
    ...
    fn process_stream(&mut self, doc: &'a Document, content: Vec<u8>, resources: &'a Dictionary, media_box: &MediaBox, output: &mut dyn OutputDev, page_num: u32) -> Result<(), OutputError> {
        ...
        for operation in &content.operations {
            ...
            // learnive patch (see vendor/pdf-extract/PATCH.md): a malformed
            // content stream can carry an operator with fewer operands than
            // it needs (observed live: a bare `w` with zero operands). This
            // whole match block used to index `operation.operands[N]`
            // unconditionally (or `assert!` the exact count), which panics
            // and — worse — since callers process a page's operations in
            // one pass, aborts extraction of every operation after it,
            // discarding otherwise-good text on the same page. Skip only
            // the malformed operation instead.
            if operation.operands.len() < min_operands(operation.operator.as_ref()) {
                warn!(
                    "skipping malformed operator {:?}: expected at least {} operand(s), got {}",
                    operation.operator,
                    min_operands(operation.operator.as_ref()),
