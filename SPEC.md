...
- **Origem**: **apenas fontes legalmente distribuíveis.** Recursos educacionais abertos (**OER** — OpenStax implementado; LibreTexts e DOAB planejados) para livros, e **arXiv** para artigos. O agente de IA decide, durante a geração, quais fontes são pertinentes para fundamentar um nó/tema e as baixa sob demanda, adicionando ao acervo da aplicação.
...
- **Cadeia de fallback, explícita ao usuário**: OER/arXiv → se nenhuma fonte adequada for encontrada, **fallback explícito para busca na internet**, sinalizado ao usuário. O conteúdo web-fundamentado é sempre atribuído inline ("segundo o site X ...", §11).
