...
**What works today:** secure server (§3.1); node parse/serialize + anchoring (§4.3); file store (§4); swappable provider with streaming + tiering + OpenRouter PKCE (§12); the loop — cold start → outline → streamed node generation → locked-rubric grading → remediation/advance (§6/§8/§8.2) — usable in the browser. Runs keyless in a **demo mode** (prompt-aware mock) when no provider is configured. Provider precedence in `build_ai` (§12): `LEARNIVE_API_BASE_URL` (+ `LEARNIVE_API_KEY`) for any OpenAI-compatible endpoint (Mercury/Inception, OpenCode Zen, local) → else `LEARNIVE_OPENROUTER_KEY` (default) → else demo. Other env: `LEARNIVE_MODEL_FAST`/`_ROBUST`, `LEARNIVE_PORT`, `LEARNIVE_DATA_DIR`, `LEARNIVE_NO_OPEN`. A local gitignored `.env` is loaded at startup (`.env.example` is the template); the real environment wins.
